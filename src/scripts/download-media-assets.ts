...
async function main() {
  ...
  const media = await AppDataSource.query('SELECT blob_storage_id FROM media WHERE id = $1', [
    MEDIA_ID,
  ]);
  ...
  const assets = await AppDataSource.query(
    'SELECT type, blob_storage_id FROM media_assets WHERE media_id = $1',
    [MEDIA_ID],
  );
  ...
  const steps = await AppDataSource.query(
    'SELECT step_key, blob_storage_id FROM media_steps WHERE media_id = $1',
    [MEDIA_ID],
  );
  ...
  for (const step of steps) {
    ...
    for (const key of blobIds) {
      if (typeof key === 'string') {
        ...
        await downloadFile(
          key,
          path.join(DOWNLOAD_DIR, 'steps', step.step_key || 'unknown', filename),
        );
      }
    }
  }
