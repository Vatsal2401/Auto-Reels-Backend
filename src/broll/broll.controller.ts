...
export class BrollController {
  ...
  @Post('videos/presign')
  @ApiOperation({
    summary: 'Get a presigned S3 PUT URL to upload a B-roll video directly from the browser',
  })
  async presignUpload(@Req() req: any, @Body() body: { filename: string; contentType?: string }) {
    if (!body.filename) throw new BadRequestException('filename is required');
    return this.brollService.presignUpload(
