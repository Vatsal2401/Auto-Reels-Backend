...
export class GeminiTTSProvider implements ITextToSpeech {
    ...
    async textToSpeech(optionsOrText: AudioOptions | string): Promise<Buffer> {
        ...
        this.logger.log(`Generating audio with Gemini TTS... Text Snippet: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}" (Length: ${text.length})`);
    ...
    private async synthesizeWithRetry(text: string, voiceName: string, fallbackVoice: string): Promise<Buffer> {
        const request = {
            input: { text: text },
        ...
        try {
            ...
            if (error.message.includes('INVALID_ARGUMENT') || error.code === 3 || error.message.includes('empty input')) {
                this.logger.warn(`Primary voice ${voiceName} failed. Falling back to permissive voice ${fallbackVoice}. Error: ${error.message}`);
    ...
    private processResponse(response: any, voiceName: string): Buffer {
        if (response && response.audioContent) {
            ...
            if (typeof response.audioContent === 'string') return Buffer.from(response.audioContent, 'base64');
