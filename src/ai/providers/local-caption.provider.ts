...
export class LocalCaptionProvider implements ICaptionGenerator {
    ...
    private readonly FILLER_WORDS = new Set([
        ...
        'literally', 'so', 'just', 'well', 'anyway', 'anyhow'
    ]);

    async generateCaptions(audioBuffer: Buffer, script?: string, captionPrompt?: string): Promise<Buffer> {
        try {
    ...
    private generateSyncedSRT(script: string, totalDuration: number): string {
        ...
        for (const word of words) {
            ...
            if (currentBlock.length >= this.MAX_WORDS_PER_CAPTION ||
