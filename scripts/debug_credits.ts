...
async function bootstrap() {
  ...
  const recentVideos = await dataSource.query(
    `SELECT * FROM media WHERE created_at >= '${today.toISOString()}' ORDER BY created_at DESC`,
  );
  ...
  if (recentVideos.length === 0) {
    ...
    const lastVideo = await dataSource.query(
  ...
  try {
    ...
    history.forEach((tx) => {
      console.log(
        `[${tx.created_at}] Type: ${tx.transaction_type} | Amount: ${tx.amount} | Balance After: ${tx.balance_after} | Desc: ${tx.description}`,
      );
    });
  } catch (error) {
